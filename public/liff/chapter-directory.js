...
async function inviteDirectoryMember(memberId){const note=prompt('ส่งคำชวน 1-2-1\n\nใส่บริบทสั้น ๆ ได้ (ไม่บังคับ):');if(note===null)return;const r=await api({action:'create-member-self-121-invite',inviteeMemberId:memberId,note,clientActionId:crypto.randomUUID()});if(!r.ok){alert(r.error||'ส่งคำชวนไม่สำเร็จ');return;}alert(r.message||'ส่งคำชวนแล้ว');closeDirectoryProfile();show('121');showMy121Page('self');}
