...
async function loadChapterHome(force=false){
  ...
  box.innerHTML=`<section class="chapter-next"><h3>ทำต่อก่อน 1–3 เรื่อง</h3>${actions.slice(0,3).join('')}</section><div class="chapter-pulse"><div><b>${r.pendingVisitors||0}</b><small>Visitor ที่ติดตาม</small></div><div><b>${r.upcomingTraining||0}</b><small>การอบรมที่กำลังมา</small></div><div><b>${r.openRequests||0}</b><small>คำขอที่กำลังดูแล</small></div></div><div class="chapter-links"><button type="button" onclick="chapterHomeGo('visitor')"><b>👥 Visitor Journey</b><small>ลงทะเบียนและติดตามแขก</small></button><button type="button" onclick="chapterHomeGo('ceu')"><b>🎓 Training & CEU</b><small>ดูรอบเรียนและแจ้งความสนใจ</small></button><button type="button" onclick="chapterHomeGo('issue')"><b>💬 Help Center</b><small>ขอความช่วยเหลือโดยไม่ต้องรู้ว่าต้องหาทีมไหน</small></button><button type="button" onclick="chapterHomeGo('progress')"><b>📊 ความก้าวหน้า</b><small>ดูคะแนนและสิ่งที่พัฒนาขึ้น</small></button></div>`;
}
