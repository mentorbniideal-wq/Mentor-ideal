...
async function getRound(db:any,roundId:string){const d=await loadDetail(db,roundId);if(!d)return errResponse('ไม่พบรอบจับคู่');const [{data:waiting},{data:blockedActive}]=await Promise.all([db.from('pairing_waitlist').select('id,status,priority_points,reason,member:members(id,name,nickname,mentor_team,profession)').eq('round_id',roundId).eq('status','waiting').order('priority_points',{ascending:false}),db.from('round_eligibility').select('member:members(id,name,nickname),reason').eq('round_id',roundId).eq('status','excluded').like('reason','รอคู่ 1-2-1%')]);return jsonResponse({ok:true,round:d.round,pairs:d.pairs,waiting:waiting||[],blockedActive:blockedActive||[],templates:MESSAGE_TEMPLATES,previews:buildPreviews(d.pairs,d.looking,String((d.round as Record<string,unknown>).message_template_key||'growth_opportunity'))});}
