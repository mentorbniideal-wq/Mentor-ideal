...
export async function handleWeekly121(p: Record<string, unknown>): Promise<Response> {
  ...
  if(action==='saveOneToOnePilotSettings'){
    const pilotIds=Array.isArray(p.pilotIds)?[...new Set((p.pilotIds as unknown[]).map(String).filter(Boolean))]:[];if(pilotIds.length>200)return errResponse('Pilot มีสมาชิกมากเกินกำหนด');const featureEnabled=Boolean(p.featureEnabled),emergencyStop=Boolean(p.emergencyStop),enforcePilotAccess=Boolean(p.enforcePilotAccess);if(enforcePilotAccess&&!featureEnabled&&!pilotIds.length)return errResponse('ต้องเลือก Pilot Member อย่างน้อย 1 คนก่อนจำกัดสิทธิ์เข้าใช้งาน');const {error}=await db.from('settings').upsert([{key:'FEATURE_ONE_TO_ONE_SYSTEM',value:String(featureEnabled)},{key:'ONE_TO_ONE_EMERGENCY_STOP',value:String(emergencyStop)},{key:'ONE_TO_ONE_ENFORCE_PILOT_ACCESS',value:String(enforcePilotAccess)},{key:'ONE_TO_ONE_PILOT_MEMBER_IDS',value:JSON.stringify(pilotIds)}],{onConflict:'key'});if(error)return errResponse(error.message);await db.from('one_to_one_status_events').insert({event_type:'pilot_settings_updated',actor_type:'admin',actor_ref:String(auth.role),metadata:{featureEnabled,emergencyStop,enforcePilotAccess,pilotCount:pilotIds.length}});return jsonResponse({ok:true,featureEnabled,emergencyStop,enforcePilotAccess,pilotCount:pilotIds.length});
  }
