...
export async function handleWeekly121(p: Record<string, unknown>): Promise<Response> {
  ...
  if(action==='sendOneToOneProfileReminder'){
    ...
    const dryRun=p.dryRun!==false,confirmed=Boolean(p.confirmed),customMessage=String(p.customMessage||'').trim().slice(0,5000),requested=Array.isArray(p.memberIds)?[...new Set((p.memberIds as unknown[]).map(String).filter(Boolean))]:[];
    ...
    if(dryRun){const eligible=rows.filter(x=>x.lineUserId&&(x.guard as Record<string,unknown>).allowed),bulkMessage=`👤 เติม Business Profile ให้พร้อมก่อน 1-2-1\n\nข้อมูลนี้ช่วยให้คู่ของคุณรู้จักธุรกิจ GAINS และ Looking for ล่วงหน้า เพื่อเตรียมคำถามที่มีคุณภาพ\n\nเปิด MY121 → โปรไฟล์\n${profileUrl}\n\nกรอกเท่าที่สะดวกและกลับมาแก้ไขได้เสมอครับ`;return jsonResponse({ok:true,dryRun:true,eligible:eligible.map(x=>({memberId:x.memberId,name:x.name,completeness:x.completeness})),suppressed:rows.filter(x=>!(x.guard as Record<string,unknown>).allowed).map(x=>({name:x.name,reason:(x.guard as Record<string,unknown>).reason})),noLine:rows.filter(x=>!x.lineUserId).map(x=>x.name),sample:eligible.length>1?bulkMessage:rows[0]?.message||'',messageCount:eligible.length});}
