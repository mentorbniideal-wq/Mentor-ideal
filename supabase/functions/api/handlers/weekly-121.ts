...
export async function handleWeekly121(p: Record<string, unknown>): Promise<Response> {
  ...
  if(action==='sendOneToOneProfileReminder'){
    ...
    await db.from('chapter_audit_events').insert({event_type:'line_profile_reminder_batch',actor_role:String(auth.role||'mc'),actor_ref:actor,subject_type:'line_delivery_batch',subject_ref:batchId,metadata:{sent,skipped,failed,total:results.length,daily_cap_override:overrideDailyCap}});
    return jsonResponse({ok:true,batchId,sent,skipped,failed,total:results.length,results});
  }

  if(action==='createManualOneToOneRound'){
    const memberAId=String(p.memberAId||''),memberBId=String(p.memberBId||''),meetingDate=String(p.meetingDate||new Date().toISOString().slice(0,10));if(!memberAId||!memberBId)return errResponse('กรุณาเลือกสมาชิกทั้ง 2 คน');if(memberAId===memberBId)return errResponse('สมาชิกคนที่ 1 และคนที่ 2 ต้องไม่ใช่คนเดียวกัน');if(!/^\d{4}-\d{2}-\d{2}$/.test(meetingDate))return errResponse('วันที่ไม่ถูกต้อง');const {data:members,error:memberError}=await db.from('members').select('id,name,is_archived').in('id',[memberAId,memberBId]);if(memberError)return errResponse(memberError.message);if((members||[]).length!==2||((members||[]) as Record<string,unknown>[]).some(x=>x.is_archived))return errResponse('ไม่พบสมาชิก Active ที่เลือก');let activeIds:Set<string>;try{activeIds=await activePairMemberIds(db,[memberAId,memberBId]);}catch(e){return errResponse(e instanceof Error?e.message:'ตรวจคู่ที่กำลังดำเนินการไม่สำเร็จ');}if(activeIds.size){const names=((members||[]) as Record<string,unknown>[]).filter(x=>activeIds.has(String(x.id))).map(x=>String(x.name));return errResponse(`ยังสร้างคู่ใหม่ไม่ได้: ${names.join(', ')} มีคู่ 1-2-1 ที่กำลังดำเนินการอยู่ กรุณาให้ MC ปล่อยคู่เดิมก่อน`);}const {data:links}=await db.from('line_members').select('member_id,line_user_id').in('member_id',[memberAId,memberBId]);const ready=new Set(((links||[]) as Record<string,unknown>[]).filter(x=>x.line_user_id).map(x=>String(x.member_id)));if(!ready.has(memberAId)||!ready.has(memberBId)){const missing=((members||[]) as Record<string,unknown>[]).filter(x=>!ready.has(String(x.id))).map(x=>String(x.name));return errResponse(`ยังไม่เชื่อม LINE: ${missing.join(', ')}`);}const start=new Date(`${meetingDate}T00:00:00+07:00`),end=new Date(start.getTime()+6*86400000+16*60*60*1000+59*60*1000);const {data:round,error:roundError}=await db.from('matching_rounds').insert({meeting_date:meetingDate,source_file_name:'Manual Pair · MC Desktop',matching_type:'manual',repeat_window_weeks:0,status:'draft',created_by:String(auth.displayName||auth.role),system_version:2,feature_flag:'one_to_one_system',message_template_key:templateKey(p.messageTemplateKey),timezone:'Asia/Bangkok',starts_at:start.toISOString(),ends_at:end.toISOString()}).select('id').single();if(roundError||!round)return errResponse(roundError?.message||'สร้างรอบไม่สำเร็จ');const roundId=String((round as Record<string,unknown>).id);const {error:eligibilityError}=await db.from('round_eligibility').insert([memberAId,memberBId].map(memberId=>({round_id:roundId,member_id:memberId,source:'manual',status:'matched',preference:'auto',reason:'MC เลือกคู่ทดสอบด้วยตนเอง'})));if(eligibilityError){await db.from('matching_rounds').delete().eq('id',roundId);return errResponse(eligibilityError.message);}const {error:pairError}=await db.from('matching_pairs').insert({round_id:roundId,position:1,member_a_id:memberAId,member_b_id:memberBId,is_locked:true,match_reason:['MC manual pair'],status:'matched'});if(pairError){await db.from('matching_rounds').delete().eq('id',roundId);return errResponse(pairError.message);}return getRound(db,roundId);
  }
  ...
  if (action === 'generateWeekly121Matches') {
    ...
    const preActiveIds=eligible.map(r=>String(r.matched_member_id));let alreadyActive:Set<string>;try{alreadyActive=await activePairMemberIds(db,preActiveIds,roundId);}catch(e){return errResponse(e instanceof Error?e.message:'ตรวจคู่ที่กำลังดำเนินการไม่สำเร็จ');}
  ...
  if (action === 'sendWeekly121Round') {
    ...
    const liffUrl=(Deno.env.get('LINE_LIFF_URL')||`${Deno.env.get('APP_URL')||'https://bni-mentor-system.vercel.app'}/liff/`).replace(/\/$/,'');
    ...
    if(testMode){
      ...
      const [{data:member},{data:link}]=await Promise.all([
        db.from('members').select('id,name,nickname').eq('id',ownMemberId).eq('is_archived',false).maybeSingle(),
    ...
    if(testMode&&testRecipient){
      ...
      await db.from('chapter_audit_events').insert({event_type:'weekly_121_card_preview_sent',actor_role:String(auth.role||'mc'),actor_ref:String(auth.email||auth.displayName||auth.role||'MC'),subject_type:'matching_round',subject_ref:roundId,metadata:{recipient_member_id:testRecipient.memberId,delivery_id:result.deliveryId||null,skipped:Boolean(result.skipped)}});
    ...
    await db.from('chapter_audit_events').insert({event_type:'weekly_121_line_batch',actor_role:String(auth.role||'mc'),actor_ref:String(auth.displayName||auth.role||'MC'),subject_type:'matching_round',subject_ref:roundId,metadata:{status,sent,failed,skipped,delivered:deliveredMembers.size,total:previews.length,test_mode:false}});
