...
export async function handleWeekly121(p: Record<string, unknown>): Promise<Response> {
  ...
  if (action === 'getWeekly121DeliveryLive') {
    ...
    const recipients=detail.pairs.flatMap((pair:Record<string,unknown>)=>((pair.members||[]) as Record<string,unknown>[]).map(member=>{const delivery=(member.delivery||{}) as Record<string,unknown>,partner=((pair.members||[]) as Record<string,unknown>[]).filter(x=>String(x.id)!==String(member.id)).map(x=>String(x.nickname||x.name||'สมาชิก')).join(', ');return{memberId:String(member.id),name:String(member.nickname||member.name||'สมาชิก'),partner,pairId:String(pair.id),status:String(delivery.status||'queued'),sentAt:delivery.sent_at||null,lastError:delivery.last_error||null,responseStatus:delivery.response_status||null};}));
    ...
    return jsonResponse({ok:true,roundId,roundStatus:String((detail.round as Record<string,unknown>).status||'draft'),total:recipients.length,counts,recipients,serverTime:new Date().toISOString()});
  }
  if (action === 'getOneToOneOverview') {
    ...
    const activePromise=deliveredPairIds.length?db.from('matching_pairs').select('id,status,archived_at,round:matching_rounds!inner(status)').in('id',deliveredPairIds).is('archived_at',null).neq('matching_rounds.status','cancelled').in('status',['matched','contacted','scheduled','confirmed_schedule','awaiting_verification','partially_verified']):Promise.resolve({data:[],error:null});
    ...
    return jsonResponse({ok:true,round:round||null,stats:{active:(activeRows||[]).length,completed:completedCount||0,waiting:waiting||0,followup:followUp||0,attention:attention||0},completedPairs:completedRows||[],activeDefinition:'weekly_121_matching_sent_to_both_members_excluding_cancelled',featureEnabled:String((round as Record<string,unknown>|null)?.feature_flag||'')==='one_to_one_system'});
  }
