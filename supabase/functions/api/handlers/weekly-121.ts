...
export async function handleWeekly121(p: Record<string, unknown>): Promise<Response> {
  ...
  if (action === 'importWeekly121Csv') {
    ...
    const { data: round, error:rErr } = await db.from('matching_rounds').insert({meeting_date:meetingDate,source_file_name:fileName,repeat_window_weeks:Number(p.repeatWindowWeeks||12),created_by:String(auth.displayName||auth.role),system_version:2,feature_flag:'one_to_one_system',timezone:'Asia/Bangkok',opt_in_closes_at:new Date(saturday.getTime()+5*60*60*1000).toISOString(),starts_at:saturday.toISOString(),ends_at:thursday.toISOString()}).select('id').single();
  ...
  if (action === 'sendWeekly121Round') {
    ...
    const previews=buildPreviews(detail.pairs,detail.looking);const guardedPreviews=await Promise.all(previews.map(async item=>({...item,guard:await evaluateNotificationGuard(db,{memberId:item.recipient.id,module:'one_to_one',category:testMode?'weekly_121_test':'weekly_121_matching',priority:'action_required'})})));
