...
export async function handleWeekly121(p: Record<string, unknown>): Promise<Response> {
  ...
  if (action === 'getOneToOneOverview') {
    ...
    const activePromise=db.from('matching_pairs').select('id,status,archived_at,round:matching_rounds!inner(status)').is('archived_at',null).neq('matching_rounds.status','cancelled').in('status',['matched','contacted','scheduled','confirmed_schedule','awaiting_verification','partially_verified']);
