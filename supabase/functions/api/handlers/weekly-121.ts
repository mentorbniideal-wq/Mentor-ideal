...
export async function handleWeekly121(p: Record<string, unknown>): Promise<Response> {
  ...
  if(action==='saveOneToOneBusinessProfile'){
    const memberId=String(p.memberId||''),description=String(p.description||'').trim(),profession=String(p.profession||'').trim(),companyName=String(p.companyName||'').trim();if(!memberId)return errResponse('memberId required');if(!description&&!profession&&!companyName)return errResponse('กรุณาใส่ข้อมูลธุรกิจอย่างน้อยหนึ่งช่อง');const {error:memberError}=await db.from('members').update({profession:profession||null,company_name:companyName||null,updated_at:new Date().toISOString()}).eq('id',memberId);if(memberError)return errResponse(memberError.message);if(description){const {error}=await db.from('biz_profiles').upsert({member_id:memberId,description,updated_at:new Date().toISOString()},{onConflict:'member_id'});if(error)return errResponse(error.message);}return jsonResponse({ok:true,memberId,description,profession,companyName});
  }
