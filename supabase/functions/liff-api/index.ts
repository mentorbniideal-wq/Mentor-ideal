...
import { generateHandshakeCode, handshakeCodeHash, oneToOneGoogleCalendarUrl, oneToOneIcs, pairStatusFromVerification, safeHashEqual, sharedTrioHandshakeCode } from '../_shared/one-to-one.ts';
...
Deno.serve(async (req: Request) => {
  ...
  if(action==='start-one-to-one-verification'){
    ...
    const isTrio=Boolean(pair.optional_member_c_id),code=isTrio?await sharedTrioHandshakeCode(pairId,pepper):generateHandshakeCode();const round=pair.matching_rounds as Record<string,unknown>;const expires=String(round.ends_at||new Date(Date.now()+7*86400000).toISOString());const hash=await handshakeCodeHash(pairId,memberId,code,pepper);
