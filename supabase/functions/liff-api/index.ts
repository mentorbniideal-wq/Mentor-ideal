...
Deno.serve(async (req: Request) => {
  ...
  if(action==='chapter-directory'){
    ...
    const filter=String(body.filter||'ready'),allowedFilters=new Set(['ready','all','saved']);
    if(!allowedFilters.has(filter))return response({ok:false,error:'ตัวกรอง Directory ไม่ถูกต้อง'},400);
    const [{data:people,error:peopleError},{data:profiles,error:profileError},{data:bookmarks,error:bookmarkError}]=await Promise.all([
      ...
      db.from('member_one_to_one_profiles').select('member_id,share_directory,business_summary,target_clients,problems_solved,primary_services,differentiators,service_area,looking_for,ideal_client,referral_trigger,good_referral,not_a_fit,before_intro_question,promise_boundaries,credibility_story,introduction_script,published_at,updated_at').eq('share_directory',true).limit(300),
      db.from('chapter_directory_bookmarks').select('subject_member_id').eq('member_id',memberId),
    ]);
    if(peopleError||profileError||bookmarkError)return response({ok:false,error:'ค้นหารายชื่อสมาชิกไม่สำเร็จ กรุณาลองใหม่'},500);
    const profileByMember=new Map(((profiles||[]) as Record<string,unknown>[]).map(row=>[String(row.member_id),row]));
    const savedIds=new Set(((bookmarks||[]) as Record<string,unknown>[]).map(row=>String(row.subject_member_id)));
    const rows=((people||[]) as Record<string,unknown>[]).map(person=>({member:person,profile:profileByMember.get(String(person.id))||null}));
    const ranked=rows.map(row=>{const result=directoryResult(row);return{score:directorySearchScore(row,query),result:{...result,matchReasons:directoryMatchReasons(row,query),isBookmarked:savedIds.has(String(result.id))}};})
      .filter(x=>x.score>0&&String(x.result.id)!==memberId&&(filter==='all'||filter==='ready'&&x.result.directoryOptIn||filter==='saved'&&x.result.isBookmarked))
      .sort((a,b)=>Number(b.result.directoryOptIn)-Number(a.result.directoryOptIn)||b.score-a.score||b.result.referralReadiness.percent-a.result.referralReadiness.percent||a.result.name.localeCompare(b.result.name,'th'));
    const results=ranked.slice(0,50).map(x=>x.result);
    await db.from('chapter_directory_events').insert({event_type:'search',result_count:results.length,has_query:Boolean(query)});
    return response({ok:true,query,filter,results,total:results.length,savedCount:savedIds.size,privacyNotice:'ข้อมูล Referral Focus แสดงเฉพาะสมาชิกที่เปิดแชร์ใน Chapter Directory'});
  }
  ...
  if(action==='chapter-directory-profile'){
    ...
    const [{data:person,error:personError},{data:profile,error:profileError},{pair}]=await Promise.all([
      ...
      db.from('member_one_to_one_profiles').select('member_id,share_directory,business_summary,target_clients,problems_solved,primary_services,differentiators,service_area,looking_for,ideal_client,referral_trigger,good_referral,not_a_fit,before_intro_question,promise_boundaries,credibility_story,introduction_script,published_at,updated_at').eq('member_id',subjectId).eq('share_directory',true).maybeSingle(),
      ownPair(),
    ]);
  ...
  if(action==='toggle-directory-bookmark'){
    const subjectId=String(body.memberId||'').trim(),remember=body.remember!==false;
    if(!subjectId||subjectId===memberId)return response({ok:false,error:'ไม่สามารถบันทึกโปรไฟล์นี้ได้'},400);
    const {data:subject}=await db.from('members').select('id').eq('id',subjectId).or('is_archived.eq.false,is_archived.is.null').maybeSingle();
    if(!subject)return response({ok:false,error:'ไม่พบสมาชิกที่เลือก'},404);
    const write=remember?db.from('chapter_directory_bookmarks').upsert({member_id:memberId,subject_member_id:subjectId,updated_at:new Date().toISOString()},{onConflict:'member_id,subject_member_id'}):db.from('chapter_directory_bookmarks').delete().eq('member_id',memberId).eq('subject_member_id',subjectId);
    const {error}=await write;if(error)return response({ok:false,error:'บันทึกรายการที่อยากนึกถึงไม่สำเร็จ'},400);
    await db.from('chapter_directory_events').insert({event_type:remember?'bookmark_added':'bookmark_removed',subject_member_id:subjectId});
    return response({ok:true,remembered:remember,message:remember?'บันทึกไว้ในคนที่อยากนึกถึงแล้ว':'นำออกจากรายการที่บันทึกแล้ว'});
  }
  ...
  if(action==='confirm-one-to-one-schedule'){
    ...
    if(confirmed){await db.from('one_to_one_schedules').update({status:'rescheduled',updated_at:now}).eq('pair_id',pairId).eq('status','proposed').neq('id',scheduleId);const pairAlreadyCompleted=['verified','late_verified'].includes(String(pair.status));if(!pairAlreadyCompleted)await db.from('matching_pairs').update({status:'confirmed_schedule'}).eq('id',pairId);await db.from('one_to_one_status_events').insert({round_id:String(pair.round_id),pair_id:pairId,member_id:memberId,event_type:'schedule_confirmed',actor_type:'member',actor_ref:memberId,metadata:{pair_status_preserved:pairAlreadyCompleted}});const partnerId=String(pair.member_a_id)===memberId?String(pair.member_b_id):String(pair.member_a_id);await notifyOneToOnePartner(db,{pairId,memberId,partnerId,type:'schedule_confirmed',message:'✅ คู่ของคุณยืนยันเวลา 1-2-1 แล้ว\nเปิด MY121 เพื่อเพิ่มนัดลง Calendar'});}
  ...
  if(action==='start-one-to-one-verification'){
    ...
    const {error}=await db.from('one_to_one_verifications').insert({pair_id:pairId,member_id:memberId,code_hash:hash,code_version:version,code_expires_at:expires,flow_started_at:new Date().toISOString()});if(error)return response({ok:false,error:error.message},400);
    await db.from('matching_pairs').update({status:'awaiting_verification'}).eq('id',pairId);const partnerIds=[pair.member_a_id,pair.member_b_id,pair.optional_member_c_id].filter(Boolean).map(String).filter(id=>id!==memberId);await Promise.all(partnerIds.map(partnerId=>notifyOneToOnePartner(db,{pairId,memberId,partnerId,type:'handshake_started',message:'🤝 สมาชิกในกลุ่ม 1-2-1 เริ่ม Digital Handshake แล้ว\nเปิด MY121 เพื่อดำเนินการหลังจบการพูดคุย'})));return response({ok:true,issued:true,code,expiresAt:expires,isTrio,message:isTrio?'กลุ่มพิเศษ 3 คนใช้รหัส 6 หลักนี้ร่วมกัน และต้องยืนยันครบทั้ง 3 คน':'แสดงรหัสนี้ให้คู่ของคุณหลังจบการพูดคุย ห้ามส่งต่อให้ผู้อื่น'});
  }
  ...
  if(action==='submit-one-to-one-code'){
    ...
    const isTrio=Boolean(pair.optional_member_c_id),partnerId=String(pair.member_a_id)===memberId?String(pair.member_b_id):String(pair.member_a_id);const [{data:own},{data:partnerVerification}]=await Promise.all([db.from('one_to_one_verifications').select('*').eq('pair_id',pairId).eq('member_id',memberId).maybeSingle(),isTrio?db.from('one_to_one_verifications').select('*').eq('pair_id',pairId).eq('member_id',memberId).maybeSingle():db.from('one_to_one_verifications').select('*').eq('pair_id',pairId).eq('member_id',partnerId).maybeSingle()]);if(!own||!partnerVerification)return response({ok:false,error:isTrio?'กรุณาเปิด Verification Flow ก่อน':'ทั้งสองฝ่ายต้องเปิด Verification Flow ก่อน'},400);
    ...
    const pepper=Deno.env.get('ONE_TO_ONE_CODE_PEPPER')||'',hashOwner=isTrio?memberId:partnerId,submitted=await handshakeCodeHash(pairId,hashOwner,code,pepper);if(!safeHashEqual(submitted,String(partnerRow.code_hash))){const attempts=Number(ownRow.attempts||0)+1;await db.from('one_to_one_verifications').update({attempts,locked_until:attempts>=5?new Date(Date.now()+15*60000).toISOString():null}).eq('id',String(ownRow.id));return response({ok:false,error:`รหัสไม่ถูกต้อง เหลือโอกาส ${Math.max(0,5-attempts)} ครั้ง`},400);}
    const now=new Date().toISOString();await db.from('one_to_one_verifications').update({verified_partner_code_at:now}).eq('id',String(ownRow.id));const {data:both}=await db.from('one_to_one_verifications').select('member_id,verified_partner_code_at').eq('pair_id',pairId);const requiredIds=[pair.member_a_id,pair.member_b_id,pair.optional_member_c_id].filter(Boolean).map(String),verifiedCount=requiredIds.filter(id=>Boolean((both||[]).find((x:Record<string,unknown>)=>String(x.member_id)===id)?.verified_partner_code_at)).length,round=pair.matching_rounds as Record<string,unknown>,allVerified=verifiedCount===requiredIds.length,status=allVerified?(new Date()>new Date(String(round.ends_at||now))?'late_verified':'verified'):verifiedCount?'partially_verified':'awaiting_verification';await db.from('matching_pairs').update({status}).eq('id',pairId);await db.from('one_to_one_status_events').insert({round_id:String(pair.round_id),pair_id:pairId,member_id:memberId,event_type:status,actor_type:'member',actor_ref:memberId,idempotency_key:`verification:${pairId}:${memberId}:v${Number(ownRow.code_version||0)}`});return response({ok:true,status,message:status==='partially_verified'?`ยืนยันสำเร็จ รอสมาชิกอีก ${requiredIds.length-verifiedCount} คน`:'ยืนยัน 1-2-1 สำเร็จแล้ว'});
  }
