-- Growth Watch: mentoring tiers + notifications table

-- 1. Add mentoring_mode to members
ALTER TABLE members
  ADD COLUMN IF NOT EXISTS mentoring_mode   TEXT        NOT NULL DEFAULT 'active',
  ADD COLUMN IF NOT EXISTS mode_changed_at  TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS mode_changed_by  TEXT;

ALTER TABLE members DROP CONSTRAINT IF EXISTS members_mentoring_mode_check;
ALTER TABLE members ADD CONSTRAINT members_mentoring_mode_check
  CHECK (mentoring_mode IN ('active', 'growth_watch'));

-- 2. Notifications table (in-app alerts, persistent)
...
DROP POLICY IF EXISTS "service_role_all_notif" ON notifications;
CREATE POLICY "service_role_all_notif" ON notifications FOR ALL TO service_role USING (true);

-- 3. Extend v_member_dashboard to expose mentoring_mode
